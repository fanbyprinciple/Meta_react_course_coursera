...
export default function CalendarScreen() {
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= days; day++) {
      ...
      const hasItems = EffortHistory.some(
        (Effort) =>
          new Date(Effort.timestamp).toDateString() === date.toDateString()
      );
      ...
      week.push(
        ...
        >
          ...
          {hasItems && <View style={styles.eventDot} />}
        </TouchableOpacity>
      );
  ...
  const rendertasksForDate = () => {
    ...
    const dayItems = EffortHistory.filter(
      (Effort) => new Date(Effort.timestamp).toDateString() === dateStr
    );

    return tasks.map((task) => {
      const Completed = dayItems.some(
        (Effort) => Effort.taskId === task.id && Effort.Completed
      );
      ...
      return (
        <View key={task.id} style={styles.taskCard}>
          ...
          <View style={styles.taskInfo}>
            ...
            <Text style={styles.taskmanhour}>{task.manhour}</Text>
            <Text style={styles.taskTime}>{task.times[0]}</Text>
          </View>
          {Completed ? (
